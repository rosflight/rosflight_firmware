 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_rosflight_aux_cmd_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               const uint8_t *type_array, const float *aux_cmd_array)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN, MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_CRC);
#else
...
/**
 * @brief Encode a rosflight_aux_cmd struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param rosflight_aux_cmd C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_rosflight_aux_cmd_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_rosflight_aux_cmd_t* rosflight_aux_cmd)
{
    return mavlink_msg_rosflight_aux_cmd_pack_status(system_id, component_id, _status, msg,  rosflight_aux_cmd->type_array, rosflight_aux_cmd->aux_cmd_array);
}
