...
static void mavlink_test_rosflight_gnss_full(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
    ...
    mavlink_rosflight_gnss_full_t packet1, packet2;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
...
#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("ROSFLIGHT_GNSS_FULL") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL) != NULL);
#endif
}
