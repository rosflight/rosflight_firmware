...
#define MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN 84
                               ...
                               uint32_t time_of_week, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t valid, uint32_t t_acc, int32_t nano, uint8_t fix_type, uint8_t num_sat, int32_t lon, int32_t lat, int32_t height, int32_t height_msl, uint32_t h_acc, uint32_t v_acc, int32_t vel_n, int32_t vel_e, int32_t vel_d, int32_t g_speed, int32_t head_mot, uint32_t s_acc, uint32_t head_acc, uint16_t p_dop, uint64_t rosflight_timestamp)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_rosflight_gnss_full_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint32_t time_of_week, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t valid, uint32_t t_acc, int32_t nano, uint8_t fix_type, uint8_t num_sat, int32_t lon, int32_t lat, int32_t height, int32_t height_msl, uint32_t h_acc, uint32_t v_acc, int32_t vel_n, int32_t vel_e, int32_t vel_d, int32_t g_speed, int32_t head_mot, uint32_t s_acc, uint32_t head_acc, uint16_t p_dop, uint64_t rosflight_timestamp)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN);
#endif
}
                                   ...
                                   uint32_t time_of_week,uint16_t year,uint8_t month,uint8_t day,uint8_t hour,uint8_t min,uint8_t sec,uint8_t valid,uint32_t t_acc,int32_t nano,uint8_t fix_type,uint8_t num_sat,int32_t lon,int32_t lat,int32_t height,int32_t height_msl,uint32_t h_acc,uint32_t v_acc,int32_t vel_n,int32_t vel_e,int32_t vel_d,int32_t g_speed,int32_t head_mot,uint32_t s_acc,uint32_t head_acc,uint16_t p_dop,uint64_t rosflight_timestamp)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
}
...
/**
 * @brief Encode a rosflight_gnss_full struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param rosflight_gnss_full C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_rosflight_gnss_full_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_rosflight_gnss_full_t* rosflight_gnss_full)
{
    return mavlink_msg_rosflight_gnss_full_pack_status(system_id, component_id, _status, msg,  rosflight_gnss_full->time_of_week, rosflight_gnss_full->year, rosflight_gnss_full->month, rosflight_gnss_full->day, rosflight_gnss_full->hour, rosflight_gnss_full->min, rosflight_gnss_full->sec, rosflight_gnss_full->valid, rosflight_gnss_full->t_acc, rosflight_gnss_full->nano, rosflight_gnss_full->fix_type, rosflight_gnss_full->num_sat, rosflight_gnss_full->lon, rosflight_gnss_full->lat, rosflight_gnss_full->height, rosflight_gnss_full->height_msl, rosflight_gnss_full->h_acc, rosflight_gnss_full->v_acc, rosflight_gnss_full->vel_n, rosflight_gnss_full->vel_e, rosflight_gnss_full->vel_d, rosflight_gnss_full->g_speed, rosflight_gnss_full->head_mot, rosflight_gnss_full->s_acc, rosflight_gnss_full->head_acc, rosflight_gnss_full->p_dop, rosflight_gnss_full->rosflight_timestamp);
}
...
static inline void mavlink_msg_rosflight_gnss_full_send(mavlink_channel_t chan, uint32_t time_of_week, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t valid, uint32_t t_acc, int32_t nano, uint8_t fix_type, uint8_t num_sat, int32_t lon, int32_t lat, int32_t height, int32_t height_msl, uint32_t h_acc, uint32_t v_acc, int32_t vel_n, int32_t vel_e, int32_t vel_d, int32_t g_speed, int32_t head_mot, uint32_t s_acc, uint32_t head_acc, uint16_t p_dop, uint64_t rosflight_timestamp)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL, buf, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
#else
    mavlink_rosflight_gnss_full_t packet;
    packet.rosflight_timestamp = rosflight_timestamp;
    packet.time_of_week = time_of_week;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL, (const char *)&packet, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_rosflight_gnss_full_send_struct(mavlink_channel_t chan, const mavlink_rosflight_gnss_full_t* rosflight_gnss_full)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_rosflight_gnss_full_send(chan, rosflight_gnss_full->time_of_week, rosflight_gnss_full->year, rosflight_gnss_full->month, rosflight_gnss_full->day, rosflight_gnss_full->hour, rosflight_gnss_full->min, rosflight_gnss_full->sec, rosflight_gnss_full->valid, rosflight_gnss_full->t_acc, rosflight_gnss_full->nano, rosflight_gnss_full->fix_type, rosflight_gnss_full->num_sat, rosflight_gnss_full->lon, rosflight_gnss_full->lat, rosflight_gnss_full->height, rosflight_gnss_full->height_msl, rosflight_gnss_full->h_acc, rosflight_gnss_full->v_acc, rosflight_gnss_full->vel_n, rosflight_gnss_full->vel_e, rosflight_gnss_full->vel_d, rosflight_gnss_full->g_speed, rosflight_gnss_full->head_mot, rosflight_gnss_full->s_acc, rosflight_gnss_full->head_acc, rosflight_gnss_full->p_dop, rosflight_gnss_full->rosflight_timestamp);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL, (const char *)rosflight_gnss_full, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN? msg->len : MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN;
        memset(rosflight_gnss_full, 0, MAVLINK_MSG_ID_ROSFLIGHT_GNSS_FULL_LEN);
    memcpy(rosflight_gnss_full, _MAV_PAYLOAD(msg), len);
#endif
}
