 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_external_attitude_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               float qw, float qx, float qy, float qz)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_MIN_LEN, MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_LEN, MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_MIN_LEN, MAVLINK_MSG_ID_EXTERNAL_ATTITUDE_LEN);
#endif
}
...
/**
 * @brief Encode a external_attitude struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param external_attitude C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_external_attitude_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_external_attitude_t* external_attitude)
{
    return mavlink_msg_external_attitude_pack_status(system_id, component_id, _status, msg,  external_attitude->qw, external_attitude->qx, external_attitude->qy, external_attitude->qz);
}
