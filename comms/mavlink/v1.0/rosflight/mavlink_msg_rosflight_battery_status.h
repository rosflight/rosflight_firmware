 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_rosflight_battery_status_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               float battery_voltage, float battery_current)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_BATTERY_STATUS_LEN);
#endif
}
...
/**
 * @brief Encode a rosflight_battery_status struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param rosflight_battery_status C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_rosflight_battery_status_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_rosflight_battery_status_t* rosflight_battery_status)
{
    return mavlink_msg_rosflight_battery_status_pack_status(system_id, component_id, _status, msg,  rosflight_battery_status->battery_voltage, rosflight_battery_status->battery_current);
}
