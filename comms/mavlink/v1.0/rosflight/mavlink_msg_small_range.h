...
#define MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN 13
#define MAVLINK_MSG_ID_187_LEN 13
#define MAVLINK_MSG_ID_187_MIN_LEN 13
                               ...
                               uint8_t type, float range, float max_range, float min_range)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_small_range_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint8_t type, float range, float max_range, float min_range)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_SMALL_RANGE_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN);
#endif
}
                                   ...
                                   uint8_t type,float range,float max_range,float min_range)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_SMALL_RANGE;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
}
...
/**
 * @brief Encode a small_range struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param small_range C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_small_range_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_small_range_t* small_range)
{
    return mavlink_msg_small_range_pack_status(system_id, component_id, _status, msg,  small_range->type, small_range->range, small_range->max_range, small_range->min_range);
}
...
static inline void mavlink_msg_small_range_send(mavlink_channel_t chan, uint8_t type, float range, float max_range, float min_range)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SMALL_RANGE, buf, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
#else
    mavlink_small_range_t packet;
    packet.range = range;
    packet.max_range = max_range;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SMALL_RANGE, (const char *)&packet, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_small_range_send_struct(mavlink_channel_t chan, const mavlink_small_range_t* small_range)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_small_range_send(chan, small_range->type, small_range->range, small_range->max_range, small_range->min_range);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SMALL_RANGE, (const char *)small_range, MAVLINK_MSG_ID_SMALL_RANGE_MIN_LEN, MAVLINK_MSG_ID_SMALL_RANGE_LEN, MAVLINK_MSG_ID_SMALL_RANGE_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_SMALL_RANGE_LEN? msg->len : MAVLINK_MSG_ID_SMALL_RANGE_LEN;
        memset(small_range, 0, MAVLINK_MSG_ID_SMALL_RANGE_LEN);
    memcpy(small_range, _MAV_PAYLOAD(msg), len);
#endif
}
