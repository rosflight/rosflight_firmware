 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_rosflight_output_raw_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint64_t stamp, const float *values)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_LEN, MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_OUTPUT_RAW_LEN);
#endif
}
...
/**
 * @brief Encode a rosflight_output_raw struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param rosflight_output_raw C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_rosflight_output_raw_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_rosflight_output_raw_t* rosflight_output_raw)
{
    return mavlink_msg_rosflight_output_raw_pack_status(system_id, component_id, _status, msg,  rosflight_output_raw->stamp, rosflight_output_raw->values);
}
