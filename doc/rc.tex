...
The logic required to construct the rc command structure is shown in figure~\ref{fig:rc_flow}.  The MAVLink structure is converted to its final form upon receiving the message, but the logic to do so is trivial, and is ommitted here for brevity.
...
\centering
  ...
  \node[node] (start) {start};
  \node[dec, below=2em of start] (off_active) {Offboard Active?};
