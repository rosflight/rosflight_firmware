...
NULL,
NULL,
NULL, USBD_CDC_GetHSCfgDesc, USBD_CDC_GetFSCfgDesc, USBD_CDC_GetOtherSpeedCfgDesc, USBD_CDC_GetDeviceQualifierDescriptor, };
    ...
    switch (req->bmRequest & USB_REQ_TYPE_MASK)
    {
        case USB_REQ_TYPE_CLASS:
            if (req->wLength != 0U)
            {
                ...
                {
                    ((USBD_CDC_ACM_ItfTypeDef*) pdev->pUserData_CDC_ACM)->Control (windex_to_ch, req->bRequest, (uint8_t*) hcdc->data[windex_to_ch],
                                                                                   req->wLength);
...
void USBD_Update_CDC_ACM_DESC (uint8_t *desc, uint8_t cmd_itf, uint8_t com_itf, uint8_t in_ep, uint8_t cmd_ep, uint8_t out_ep, uint8_t str_idx)
{
    desc += 9;
    for (uint8_t i = 0; i < NUMBER_OF_CDC; i++)
    {
