...
ConfigManager::ConfigResponse TestBoardConfigManager::check_config_change(device_t device, hardware_config_t config, const ConfigManager &cm) const
{
  (void)cm;
...
void TestBoardConfigManager::get_device_name(device_t device, char (&name)[BoardConfigManager::DEVICE_NAME_LENGTH]) const
{
  std::string device_name = "device #" + std::to_string(static_cast<int>(device));
  strcpy(reinterpret_cast<char*>(name), device_name.c_str());
}
void TestBoardConfigManager::get_config_name(device_t device, hardware_config_t config, char (&name)[BoardConfigManager::CONFIG_NAME_LENGTH]) const
{
