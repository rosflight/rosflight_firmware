...
TEST_F (StateMachineTest, ArmWithSticksByDefault)
{
  EXPECT_EQ(rf.state_manager_.state().armed, false);
  uint16_t rc_values[8];
  for (int i = 0; i < 8; i++)
...
TEST_F (StateMachineTest, DontArmWithSticksWhenUsingSwitch)
{
...
TEST_F (StateMachineTest, DisarmWithSticksByDefault)
{
  rf.state_manager_.set_event(StateManager::EVENT_REQUEST_ARM);
  EXPECT_EQ(rf.state_manager_.state().armed, true);
  uint16_t rc_values[8];
...
TEST_F (StateMachineTest, DontDisArmWithSticksWhenUsingSwitch)
{
  rf.params_.set_param_int(PARAM_RC_ARM_CHANNEL, 4);
  rf.state_manager_.set_event(StateManager::EVENT_REQUEST_ARM);
  uint16_t rc_values[8];
