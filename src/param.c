...
void set_param_defaults(void)
{
    ...
    init_param_float(PARAM_MOTOR_IDLE_THROTTLE, "MOTOR_IDLE_THR", 0.1); // Idle PWM sent to motors at zero throttle (Set above 0.1 to spin when armed) | 0.0 | 1.0
    init_param_int(PARAM_MOTOR_MAX_PWM, "MAX_PWM", 2000); // Maximum PWM signal sent to motors (us) | 1000 | 2100
    init_param_int(PARAM_MOTOR_MIN_PWM, "MOTOR_MIN_PWM", 1000); // Minimum PWM signal sent to motors (us) | 900 | 2000
    ...
    init_param_int(PARAM_RC_THROTTLE_OVERRIDE_CHANNEL, "RC_THR_OVRD_CHN", 4); // RC switch channel mapped to throttle override [0 -indexed] | 4 | 7
