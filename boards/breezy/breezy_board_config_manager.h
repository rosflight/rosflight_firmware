...
public:
  ...
  ConfigManager::ConfigResponse check_config_change(device_t device,
                                                    hardware_config_t config,
                                                    const ConfigManager &cm) const override;
  void get_device_name(device_t device, char (&name)[BoardConfigManager::DEVICE_NAME_LENGTH]) const override;
  void get_config_name(device_t device,
                       hardware_config_t config,
                       char (&name)[BoardConfigManager::CONFIG_NAME_LENGTH]) const override;
};
} // namespace rosflight_firmware
