...
/** RX buffer for USB */
// PTT added attributes
__attribute__((section("dma_ram")))
__attribute__((aligned(32))) static uint8_t RX_Buffer[NUMBER_OF_CDC][APP_RX_DATA_SIZE];

/** TX buffer for USB, RX buffer for UART */
// PTT added attributes __attribute__((section("dma_ram"))) __attribute__((aligned(32))) static
// Not used: uint8_t TX_Buffer[NUMBER_OF_CDC][APP_TX_DATA_SIZE]; // PTT put this in DMA RAM
...
static int8_t CDC_Receive(uint8_t cdc_ch, uint8_t * Buf, uint32_t * Len)
{
  ...
  // PTT comment out the above and replace with :
...
static int8_t CDC_TransmitCplt(uint8_t cdc_ch, uint8_t * Buf, uint32_t * Len, uint8_t epnum)
{
  //PTT uncomment the line below when we are done testing the interface.
