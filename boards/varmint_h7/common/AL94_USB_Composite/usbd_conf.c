...
// PTT NOTE! Faux_ is used instead of the Cube generated ST file that does not check maxpacket
HAL_StatusTypeDef Faux_HAL_PCD_EP_Transmit(PCD_HandleTypeDef * hpcd, uint8_t ep_addr, uint8_t * pBuf, uint32_t len)
{
  ...
  if (ep->maxpacket == 0) return HAL_ERROR; // NOTE This prevents transmitting if buffers aren't initialized.

  if (hpcd->Init.dma_enable == 1U) { ep->dma_addr = (uint32_t) pBuf; }
